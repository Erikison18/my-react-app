...
function App() {
    ...
    async function onClick() {
        ...
        let filterData = (getListPatch.data.strategy_list || []).filter((item) => {
            ...
            //     myReturn = false;
            // } else if (parseInt(item.real_days) < 30) {
            //     myReturn = false;
            // } else if (parseInt(item.max_withdraw) > 20) {
            //     myReturn = false;
            // } else if (parseInt(item.real_return) < 5) {
            //     myReturn = false;
            ...
            //     ids.push(item.id);
            //     myReturn = true;
            // } else if (((item.tag.includes('大盘') || item.tag.includes('沪深300')) && item.cnt >= 2 && item.cnt <= 5 && parseInt(item.annual_return) > 40 && item.score > 75)) {
            //     myReturn = true;
